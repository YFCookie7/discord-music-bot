    ...
    async def button_callback(self, button, interaction):
        ...
        voice.play(
            discord.FFmpegPCMAudio(
                executable=FFMPEG_PATH, source=audio_dir + audio_filename
            )
        )
...
class Search(commands.Cog):
    ...
    async def search(
        self,
        ctx,
        keyword: discord.Option(str, description="Search youtube", required=True),
    ):
        await ctx.defer()
        ...
        for i in range(0, 3):
            ...
            embed.add_field(
                name="Duration", value=search_results[i].duration, inline=True
            )
